...
class LobbyManager:
    ...
    def join_lobby(self, lobby_id: str, player_id: str, name: str) -> dict:
        lobby = self._require_lobby(lobby_id)
        if lobby.in_game:
            raise ValueError("Lobby already in game")
    ...
    def leave_lobby(self, lobby_id: str, player_id: str) -> dict:
        lobby = self._require_lobby(lobby_id)
        lobby.players.pop(player_id, None)
        if lobby.engine and player_id in lobby.engine.players:
            # Mark player inactive in-engine if already started
            lobby.engine.players[player_id].is_active = False
